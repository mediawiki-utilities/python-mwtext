...
::
    ...
    Converts a word2vec formatted file to a pair of gensim binaries
    containing the same vectors.  This script creates two output
    files: one file as configured by the <output> arg and another with
    ".vectors.numpy" added to the end of what is specified in the 
    <output> arg.
...
def word2vec2gensim(input_path, limit, output_path, verbose):
    model = KeyedVectors.load_word2vec_format(input_file, limit=limit)
